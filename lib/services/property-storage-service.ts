...
export class PropertyStorageService {
    ...
    private async syncToSupabase(property: IntelligentPropertyResult, slug: string): Promise<void> {
        ...
        console.log(`[Storage] 📤 Attempting to sync with Supabase...`)

        try {
            // Transform property to match DB schema
            // Note: id is auto-generated by database (identity column)
            const dbRecord = {
                name: property.title, // ← Renamed: title → name
            ...
            console.error(`[Storage] ❌ Supabase sync exception:`)
            console.error(`[Storage] 🔴 Exception type:`, err?.constructor?.name)
            console.error(`[Storage] 🔴 Exception message:`, err?.message)
            console.error(`[Storage] 🔴 Exception cause:`, err?.cause)
            console.error(`[Storage] 🔴 Exception stack:`, err?.stack)
            console.error(`[Storage] 🔴 Full exception:`, err)
        }
    }
